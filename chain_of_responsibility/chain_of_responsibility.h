#ifndef OSW_CHAIN_OF_RESPONSIBILITY_H
#define OSW_CHAIN_OF_RESPONSIBILITY_H
...
namespace chain_of_responsibility {
    ...
    public:
        ...
        explicit request_with_command(command_pattern::command *target_action) :
                _target_action(target_action), _date_time_activity_started(0) {}
    ...
    public:
        bool handle(std::string const &request, std::int64_t current_time) final {
            ...
            _date_time_activity_started = current_time;
            write_time_to_file(request);
         ...
         public:
             ...
             void write_time_to_file(std::string const& request) const
             {
                 ...
                 char buffer[80];
                 if (std::strftime(buffer, sizeof(buffer), "[%Y-%m-%d %H:%M:%S]", tm_info) == 0) {
                     std::cerr << "ERROR: the time string could not be formatted!" << std::endl;
                 }
...
#endif //OSW_CHAIN_OF_RESPONSIBILITY_H
